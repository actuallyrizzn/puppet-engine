...
def test_get_token_price(trader):
    with patch.object(trader, 'get_token_price', return_value=42.0):
        result = trader.get_token_price('SOL')
        assert result == 42.0

def test_get_supported_tokens(trader):
    with patch.object(trader, 'get_supported_tokens', return_value=['SOL', 'USDC']):
